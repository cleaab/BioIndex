...
#' @param cutoff cutoff value for splitting spawner portion of population
...
index_spawn <- function(mTATB,mTATC, GSA, country, depth_range, cutoff, stratification, wd=NA, save=TRUE) {
    ...
    ddd <- merge_TATC[merge_TATC$LENGTH_CLASS >= threshold  & merge_TATC$SEX ==sex  ,] # & merge_TATC$LENGTH_CLASS != -1
    ...
    if (nrow(ddd) != 0) {
        ...
        main <- paste(sspp,"_GSA",GSA,"_(abundance of spawners)-Random_Stratified_Sampling_",depth_range[1],"-",depth_range[2], " m", sep="")
        ...
        main <- paste(sspp,"_GSA",GSA,"_(abundance of spawners)-Random_Stratified_Sampling_",depth_range[1],"-",depth_range[2], " m", sep="")
        main.lab <- paste(sspp," GSA",GSA," (abundance of spawners)-RSS ",depth_range[1],"-",depth_range[2], " m", sep="")
