...
index_ts_F <- function(mTATB, GSA, country_analysis, depth_range, strata_scheme, stratification, wd=NA, save=TRUE) {
    ...
    for(i in 1:length(res_table_cala2[,1])) {
        ...
        if (analysis_stratum2 == T){
            s_n <- sum(s2[!is.na(s2$NB_OF_FEMALES),"NB_OF_FEMALES"])
        ...
        if (analysis_stratum3 == T){
            s_n <- sum(s3[!is.na(s3$NB_OF_FEMALES),"NB_OF_FEMALES"])
        ...
        if (analysis_stratum4 == T){
            s_n <- sum(s4[!is.na(s4$NB_OF_FEMALES),"NB_OF_FEMALES"])
        ...
        if (analysis_stratum5 == T){
            #index computation
            s_n <- sum(s5[!is.na(s5$NB_OF_FEMALES),"NB_OF_FEMALES"])
        ...
        if (analysis_stratum6 == T){
            #index computation
            s_n <- sum(s6[!is.na(s6$NB_OF_FEMALES),"NB_OF_FEMALES"])
        ...
        positive_hauls[i] <- length(merge_TATB[merge_TATB$NB_OF_FEMALES>0 & merge_TATB$YEAR == res_table_cala2[i,1],"NB_OF_FEMALES"])
    ...
    main <- paste(sspp,"_GSA",GSA,"_(",dependent,")-FEMALES-Random_Stratified_Sampling_",depth_range[1],"-",depth_range[2], " m", sep="")
    main.lab <- paste(sspp," GSA",GSA," (",dependent,")-FEMALES-RSS ",depth_range[1],"-",depth_range[2], " m", sep="")
